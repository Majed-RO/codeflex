...
http.route({
	path: '/clerk-webhook', // the same sufix we added to the clerk webhook endpoint
	method: 'POST',
	handler: httpAction(async (ctx, request) => {
		...
		if (eventType === 'user.created') {
			...
			try {
				// create new record in the users table with
    ...
    if(eventType === 'user.updated') {
      ...
      try {
        // update record in the users table with
	...
	handler: httpAction(async (ctx, request) => {
		try {
			...
			const model = genAi.getGenerativeModel({
				...
				generationConfig: {
					temperature: 0.4, // lower temperature for more focused responses / more predictable outputs
					topP: 0.9, // use top-p sampling for diversity
...
http.route({
	...
	handler: httpAction(async (ctx, request) => {
		...
		console.log('Payload OF TEST is here:', payload);
